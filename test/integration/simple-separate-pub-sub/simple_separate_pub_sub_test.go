...
package simple_separate_pub_sub
...
func TestSimpleSeparatePubSub(t *testing.T) {
	...
	bpt.DefineVerify(func(assert *assert.Assertions) {
		...
		op = gcloud.Runf(t, "pubsub subscriptions describe sub-pull --project=%s", projectId)
		assert.Equal(fmt.Sprintf("projects/%s/subscriptions/sub-pull", projectId), op.Get("name").String(), "has expected name")
		...
		op = gcloud.Runf(t, "pubsub subscriptions describe sub-push --project=%s", projectId)
		assert.Equal(fmt.Sprintf("projects/%s/subscriptions/sub-push", projectId), op.Get("name").String(), "has expected name")
		assert.Equal(fmt.Sprintf("projects/%s/topics/cft-tf-pub-topic", projectId), op.Get("topic").String(), "has expected topic")
		...
		op = gcloud.Runf(t, "pubsub subscriptions describe sub-pull2 --project=%s", projectId)
		...
		op = gcloud.Runf(t, "pubsub schemas describe pub-example --project=%s", projectId)
		assert.Equal(fmt.Sprintf("projects/%s/schemas/pub-example", projectId), op.Get("name").String(), "has expected name")
