...
package bigquery_separate_pub_sub
...
func TestBigquerySeparatePubSub(t *testing.T) {
	...
	bpt.DefineVerify(func(assert *assert.Assertions) {
		...
		op := gcloud.Runf(t, "pubsub topics describe cft-tf-pub-topic-bigquery --project=%s", projectId)
		assert.Equal(fmt.Sprintf("projects/%s/topics/cft-tf-pub-topic-bigquery", projectId), op.Get("name").String(), "has expected name")
		...
		op = gcloud.Runf(t, "pubsub subscriptions describe sub_example_subscription --project=%s", projectId)
		assert.Equal("", op.Get("RetryPolicy.minimumBackoff").String(), "has expected minimum_backoff")
		assert.Equal(fmt.Sprintf("projects/%s/subscriptions/sub_example_subscription", projectId), op.Get("name").String(), "has expected name")
		assert.Equal(fmt.Sprintf("projects/%s/topics/cft-tf-pub-topic-bigquery", projectId), op.Get("topic").String(), "has expected topic")
		...
		assert.Equal(fmt.Sprintf("%s:sub_example_dataset.sub_example_table", projectId), op.Get("bigqueryConfig.table").String(), "has expected table")
